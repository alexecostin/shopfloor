...
export const listTemplates = wrap(async (req, res) => {
  ...
  res.json(await service.listTemplates({ machineType, isActive: isActive === 'true' ? true : isActive === 'false' ? false : undefined }, req));
});
...
export const createTemplate = wrap(async (req, res) => {
  res.status(201).json(await service.createTemplate(req.body, req));
});
...
export const complete = wrap(async (req, res) => {
  res.status(201).json(await service.completeChecklist(req.body, req.user.userId, req));
});

export const listCompletions = wrap(async (req, res) => {
  const { machineId, operatorId, templateId, page, limit } = req.query;
  res.json(await service.listCompletions({ machineId, operatorId, templateId, page: +page || 1, limit: +limit || 50 }, req));
});
